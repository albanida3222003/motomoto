...
import { categories, promotions, restaurants, dataLoaded } from './data.js';
import { shippingFor } from './distance.js';
import { openItemOptions } from './options.js';
...
export function renderRestaurants() {
  ...
  if (!dataLoaded) {
    grid.innerHTML = '<div style="grid-column: 1/-1; text-align:center; padding:30px; color:var(--text-muted);">Cargando restaurantes…</div>';
    return;
  }
  ...
  if (filtered.length === 0) {
    const msg = restaurants.length === 0
      ? 'Aún no hay restaurantes cargados. Agrega locales desde el panel admin.'
      : 'No se encontraron restaurantes o platillos que coincidan.';
    grid.innerHTML = `<div style="grid-column: 1/-1; text-align:center; padding:30px; color:var(--text-muted);">${msg}</div>`;
    return;
  }
  ...
  filtered.forEach(r => {
    ...
    card.innerHTML = `
      <div class="rest-img" style="background-image:url('${r.img}')">
        <span class="rest-status-pill ${r.atendiendo ? 'on' : 'off'}">${r.atendiendo ? 'Atendiendo' : 'Cerrado'}</span>
      </div>
      <div class="rest-body">
        <div class="rest-top">
          <div class="rest-title">${r.name}</div>
        </div>
        <div class="rest-desc">${r.categoryLabel}</div>
        <div class="rest-meta">
...
export function openMenu(id) {
  ...
  if (!r.atendiendo) {
    showToast(`${r.name} no está atendiendo en este momento`);
  }
  ...
  if (r.menu.length === 0) {
    list.innerHTML = '<div style="text-align:center; padding:24px; color:var(--text-muted);">Este local aún no tiene platos disponibles.</div>';
  }

  r.menu.forEach(m => {
    const el = document.createElement('div');
    el.className = 'menu-item';
    const hasOptions = (m.gruposOpciones || []).length > 0;
    el.innerHTML = `
      <img src="${m.img}" alt="${m.name}">
      <div class="menu-item-info">
        <div class="menu-item-name">${m.name}</div>
        <div class="menu-item-desc">${m.desc}</div>
        <div class="menu-item-price">S/ ${m.price.toFixed(2)}${hasOptions ? ' +' : ''}</div>
      </div>
      <button class="add-btn">Agregar</button>`;
    el.querySelector('.add-btn').addEventListener('click', () => openItemOptions(r.id, m.id));
    list.appendChild(el);
  });
